function agressividade()
    ...
    % (1) Bloom Filter
    vetor_binario = bloom_filter(mensagens_a_analisar); % Chama a função bloom_filter que retorna o vetor binário

    % (2) minHash
    mensagens_treino = 'aggressive_texts.json';    % Mensagens que só contém asneiras
    mensagens_sem_similaridade = minHash(vetor_binario, mensagens_a_analisar, mensagens_treino);   % Retorna mensagens que não encontraram similaridade
